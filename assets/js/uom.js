var options={
    url: base_url+"productuom/load_grid",
    method:"POST",
    pagePosition:"top",
    resizeHandle:"right",
    resizeEdge:10,
    ...
    toolbar:[{
        ...
        handler: function(){
            getParamOption("dg", function (x, x1, x2) {
                let urlss = base_url+"productuom/export_data?field="+x+"&op="+x1+"&value="+x2;
    ...
    columns:[[
        // {field:"uom_code",   title:"ID",      width: '10%', sortable: true},
        {field:"uom_id",   title:"Kode",      width: 70, sortable: true},
...
function addnew(){
    ...
    $('#uom_code').textbox({disabled:true, readonly:false, width:'100%'});
    $('#submit').linkbutton({disabled:false});
    $('#cancel').linkbutton({disabled:false});
    $('#fm').form('clear');
    flag = "productuom/save_data";
}
function editData(){
    let row = getRow();
    if(row==null) return
    ...
    $.ajax({
        type:"POST",
        url:base_url+"productuom/read_data/"+row.uom_code,
        dataType:"html",
...
function ApproveData(){
    ...
    $.messager.confirm('Confirm',`Are you sure you want to ${row.status==='Draft'?'Approve':'set to Draft'} this item?`,function(r){
        if (r){
            $.ajax({
                type:"POST",
                url:base_url+"productuom/edit_data_status/"+row.uom_code+"/"+par,
                dataType:"html",
                success:function(result){
                    var data = $.parseJSON(result);
                    if (data.status === 0) {
...
function deleteData(){
    ...
    $.messager.confirm('Confirm','Are you sure you want to destroy this data?',function(r){
        if (r){
            $.post(
                base_url+"productuom/delete_data/"+row.uom_code,function(result){
