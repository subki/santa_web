...
$(document).ready(function () {
	...
	$('#no_faktur').textbox('textbox').bind('keyup',function (e) {
		console.log("Cek1",e.target.value.length)
		if(e.target.value.length<=9){
			cekFaktur();
		}else{
			var x = e.target.value;
			if(!x.startsWith('SGI.'+y+'.') && !x.startsWith('IVS.'+y+'.')){
				cekFaktur()
			}
		}
	});
	$('#no_faktur2').textbox('textbox').bind('keyup',function (e) {
		console.log("Cek2",e.target.value.length)
		if(e.target.value.length<=9){
			cekFaktur();
		}else{
			var x = e.target.value;
	...
	if(aksi==="add"){
		...
		flag = "wholesales/edit_data_header";
		$.ajax({
			type:"POST",
...
function initHeader() {
	...
	if(so_item.no_faktur.length>9){
		$("#no_faktur").textbox('readonly',!so_item.no_faktur.startsWith("SGI."));
	}
	if(so_item.no_faktur2.length>9){
		$("#no_faktur2").textbox('readonly',!so_item.no_faktur2.startsWith("SGI."));
	}
	if(so_item.no_faktur.length>9 && so_item.no_faktur2.length>9){
		$("#crt_faktur").hide();
	}else{
...
function verifyFA() {
	$.ajax({
		type:"POST",
		url:base_url+"wholesales/update_finance_verify",
		dataType:"json",
		data:so_item,
		success:function(result){
			console.log(result.data)
...
function checkedFP(isChecked) {
	if($("#pkp").textbox('getValue')==="YES"){
		...
		if(isChecked){
			if(aksi==="add"){
				var x = $("#no_faktur").textbox('getValue');
				if(!x.startsWith('SGI.'+y+'.') && !x.startsWith('IVS.'+y+'.')){
			...
			if(aksi==="add"){
				...
				if(so_item!==undefined){
					...
					if(so_item.no_faktur2.length<=9){
						...
						var x = $("#no_faktur2").textbox('getValue');
						if(!x.startsWith('SGI.'+y+'.') && !x.startsWith('IVS.'+y+'.')){
							$("#no_faktur2").textbox('setValue',so_item.no_faktur)
						}
					}
				}
			}
...
function createSeriPajak() {
	...
	$.ajax({
		type:"POST",
		url:base_url+"wholesales/get_seripajak",
		dataType:"json",
		data:so_item,
		success:function(result){
			console.log(result.data)
...
function printSOss() {
	window.open(base_url+'wholesales/print_so/'+docno, '_blank');
}
...
function printSO() {
	...
	$.messager.confirm({
		...
		fn: function(r){
			if (r){
				if(print_selected.label==="2" && so_item.status==="OPEN" && so_item.payment_first==="Yes"){
					$.messager.alert("Alert","Surat Jalan tidak bisa di print sebelum Customer melakukan pembayaran dan status faktur Posting.")
					return
				}
				let urlss = `${base_url}wholesales/print_ws?id=${so_item.id}&tipe=${print_selected.label}&pkp=${so_item.pkp}`;
...
function initGrid() {
	...
	$("#dg").datagrid({
		...
		loadFilter: function (data) {
			...
			if(data.rows.length>0)$("#clear_faktur").show();
			else $("#clear_faktur").hide();
			return data;
		},
		...
		onBeginEdit: function (index, row) {
			if (row.isNewRecord ){
				if(so_item.status!=="OPEN") {
					$.messager.show({
						title: 'Warning',
						msg: "Wholesales sudah di posting"
					});
					setTimeout(function () {
...
function submit(stt){
	...
	if(aksi==="add"){
		...
		if(so_item!==undefined){
			if(so_item.status==="CLOSED" && (status==="OPEN" || status==="APPROVED CR" || status==="APPROVED MS")){
				...
				// myConfirm("Confirm", "Anda yakin ingin mengubah status wholesales ini?", "Yes", "No", function (r) {
				//     console.log(r)
				//     if (r === "Yes") {
				//         submit_reason("")
				//     }
				...
				myConfirm("Confirm", "Anda yakin ingin mengubah status sales ini?", "Yes", "No", function (r) {
					if (r === "Yes") {
						...
						// 	var noo = "Rp. "+numberFormat(parseFloat(max_transaksi));
						// 	var psn = 'Nilai transaksi lebih dari '+noo+', dibutuhkan otorisasi untuk memposting.';
						// 	$.messager.show({title: 'Error', msg: psn});
						// 	$('#status').textbox('setValue', so_item.status);
						}else{
							submit_reason("")
				...
				if (res.status === 0) {
					...
					if(stt==="POSTING") {
						myConfirm("Success", "Posting berhasil, apakah anda ingin mencetak doc.?", "Cetak", "Tidak", function (r) {
							if (r === "Cetak") {
								printSO()
								window.location.href = base_url + "wholesales/form/edit?id=" + res.id
							}else window.location.href = base_url + "wholesales/form/edit?id=" + res.id
...
function populateBaseSO() {
	$('#base_so').combogrid({
      ...
      if(parseInt(rw.stid)>0){
			    $.messager.alert("Warning","Nomor Packing List Sudah terbentuk di Wholesales : "+rw.stno);
				$('#base_so').combogrid('setValue',null);
				return;
      }
			$('#remark').textbox('setValue',rw.docno+' '+rw.remark)
			$('#customer_name').textbox('setValue',rw.customer_name)
