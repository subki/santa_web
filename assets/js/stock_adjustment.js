...
var options={
    ...
    saveUrl:base_url+"stockadjustment/save_data",
    updateUrl:base_url+"stockadjustment/edit_data",
    destroyUrl:base_url+"stockadjustment/delete_data",
    ...
    columns:[[
        {field:"docno",   title:"Nomor Adjustment",      width: '14%', sortable: true, editor:{
                type:'textbox',
                options:{
                    readonly:true
                }
            }},
        {field:"doc_date",   title:"Tanggal",      width: '15%', sortable: true, formatter:function (value, row) {
            return row.ak_doc_date;
        },
            editor:{type:'datebox', options:{required:true, readonly:false}}},
        {field:"from_store_code",   title:"Store",      width: '22%', sortable: true, formatter: function(value, row){
        ...
        {field:"from_location_code",   title:"Location",      width: '22%', sortable: true, formatter: function(value, row){
        ...
        {field:"receive_date",   title:"Tanggal Approve",      width: '15%', sortable: true, formatter:function (value, row) {
            return row.ak_receive_date;
        },
            editor:{type:'datebox'}},
        {field:"status",   title:"Status",      width: '12%', sortable: true, editor:{
                type:'combobox',
                options:{
                    valueField:'status',
                    textField:'name',
...
function openBarangDO() {
    ...
    $("#dd").edatagrid({
        url: base_url+"stockadjustment/load_grid_nobar/"+row.docno,
        saveUrl: base_url+"stockadjustment/save_data_nobar/"+row.docno,
        updateUrl: base_url+"stockadjustment/edit_data_nobar",
        destroyUrl: base_url+"stockadjustment/delete_data_nobar",
        fitColumns:true,
        idField:'id',
        height:'100%',
        singleSelect:true,
...
function fields(row) {
    ...
    ff.push({field:"qty",   title:"Qty Adjustment",      width: '10%', sortable: true, formatter:numberFormat, editor:{
        type:'numberbox',
        options:{disabled:false, readonly:qty_send, required:r_send}}});
    ff.push({field:"qty_rcv",   title:"Qty Approved",      width: '10%', sortable: true, formatter:numberFormat, editor:{
        type:'numberbox',
