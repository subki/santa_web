...
$(document).ready(function () {
 ...
 $('#so_no').textbox('textbox').focus();   
    ...
    if(aksi==="add"){
        flag = "Showroomsales/save_data_header";
        var date = new Date();
        ...
        flag = "Showroomsales/edit_data_header";
        $.ajax({
            type:"POST",
            url:base_url+"Showroomsales/read_data/"+docno,
            dataType:"json",
             ...
             if(result.total >=1){  
                ...
                if(result.status===0) {
                    ...
                    $.messager.show({
                        ...
                        msg: e.message,
                        handler:function () {
                            window.location.href = base_url+"Showroomsales";
                        }
                    });
                }
...
function addform() {
         window.location.href = base_url+"Showroomsales/form/add?customer_code="+$('#customer_code').combogrid('getValue')+"&customer_name="+$('#customer_name').textbox('getValue');
   // window.open(base_url+'Showroomsales/print_so/'+docno, '_blank');
}
// function printSO() {
//         window.open(base_url+'Showroomsales/print_so/'+docno, '_blank', 'location=yes,height=400,width=500,scrollbars=yes,status=yes');
//    // window.open(base_url+'Showroomsales/print_so/'+docno, '_blank');
// }
// function printSO() {
...
//     $.ajax({
//         type:"get",
//         url:base_url+"Showroomsales/print_so/"+docno,
//         dataType:"json",
//         success:function(result){
...
function initGrid() { 
    ...
    $("#dg").edatagrid({ 
        fitColumns: false,
        width: "100%",
        url: base_url + "Showroomsales/load_grid_detail/"+so_item.docno,
        saveUrl: base_url + "Showroomsales/save_data_detail/"+so_item.docno,
        updateUrl: base_url + "Showroomsales/edit_data_detail",
        destroyUrl: base_url + "Showroomsales/delete_data_detail",
        idField: 'id',
        method: "POST",
        pagePosition: "top",
        resizeHandle: "right",
                        ...
                        loadFilter: function (data) {
                         ...
                         inputEvents:$.extend({},$.fn.textbox.defaults.inputEvents,{
                            ...
                            if(e.key==='Enter' || e.keyCode===13){ 
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: `${base_url}Showroomsales/get_byproduct`, 
                                    data: {
...
function get_unit_price(row, index, callback) { 
    $.ajax({
        type:"POST",
        url: `${base_url}Showroomsales/get_unit_price?product_id=${row}&tanggal=${so_item.doc_date}&lokasi=${so_item.lokasi_stock}&customer_code=${so_item.customer_code}`,
        dataType:"json",
        success:function(result){
         console.log(result)
            ...
            if(r==="Ya"){
                $.ajax({
                    ...
                    success:function(result){
                        //console.log(result.data)
                        if(result.status===0) {
                            window.location.href = base_url + "Showroomsales/form/edit?docno=" + so_item.docno
                        }
                        else {
                            $.messager.show({
                                title: 'Error',
...
function openCopy() {
    ...
    $("#combo").combogrid({
        ...
        required:true,
        readonly:false,
        url:base_url+"Showroomsales/load_grid",
        hasDownArrow: false,
        remoteFilter:true,
        panelWidth: 500,
        multiple:false,
...
function submitCopy() {
    ...
    //console.log(xx)
    $.ajax({
        url: base_url+"Showroomsales/copy_detail",
        type: 'post',
        data: {
            from:xx,
            to:so_item.docno
          ...
          //  console.log(result)
            try {
                ...
                if (res.status === 0) {
                    ...
                    if(stt=="ON ORDER") {
                        printSO()
                        window.location.href = base_url + "Showroomsales/form/edit?docno=" + res.docno
                    }else{
                        window.location.href = base_url + "Showroomsales/form/edit?docno=" + res.docno
                    }
                } else {
...
function infoData() {
    ...
    if(aksi==="edit"){
        $.ajax({
            type:"POST",
            url:base_url+"Showroomsales/read_history/"+so_item.docno,
            dataType:"json",
            success:function(result){
                //console.log(result.data)
                if(result.status===0) {
