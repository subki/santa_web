var options={
    url: base_url+"uomconvertion/load_grid",
    method:"POST",
    pagePosition:"top",
    resizeHandle:"right",
    resizeEdge:10,
    ...
    toolbar:[{
        ...
        handler: function(){
            getParamOption("dg", function (x, x1, x2) {
                let urlss = base_url+"uomconvertion/export_data?field="+x+"&op="+x1+"&value="+x2;
    ...
    columns:[[
        {field:"code1",   title:"UOM From",      width: 120, sortable: true},
        {field:"code2",   title:"UOM To",      width: 120, sortable: true},
        {field:"convertion",   title:"Nilai Konversi",      width: 70, sortable: true},
...
function editData(){
    ...
    $.ajax({
        type:"POST",
        url:base_url+"uomconvertion/read_data/"+row.uom_from+"/"+row.uom_to,
        dataType:"html",
...
function ApproveData(){
    ...
    $.messager.confirm('Confirm',`Are you sure you want to ${row.status==='Draft'?'Approve':'set to Draft'} this item?`,function(r){
        if (r){
            $.ajax({
                type:"POST",
                url:base_url+"uomconvertion/edit_data_status/"+row.id+"/"+par,
                dataType:"html",
                success:function(result){
                    var data = $.parseJSON(result);
                    if (data.status === 0) {
...
function deleteData(){
    ...
    $.messager.confirm('Confirm','Are you sure you want to destroy this data?',function(r){
        if (r){
            $.post(
                base_url+"uomconvertion/delete_data/"+row.uom_from+"/"+row.uom_to,function(result){
