...
$(document).ready(function () {
   ...
   // populateBaseSO();
    ...
    if(aksi==="add"){
        flag = "Rekapoutlet/save_data_header";
        var date = new Date();
        ...
        flag = "Rekapoutlet/edit_data_header";
        $.ajax({
            type:"POST",
            url:base_url+"Rekapoutlet/read_data/"+docno,
            dataType:"json",
            success:function(result){
                ...
                if(result.status===0) {
                    ...
                    $.messager.show({
                        ...
                        msg: result.msg,
                        handler:function () {
                            window.location.href = base_url+"Rekapoutlet";
                        }
                    });
                }
...
function verifyFA() {
    $.ajax({
        type:"POST",
        url:base_url+"Rekapoutlet/update_finance_verify",
        dataType:"json",
        data:so_item,
        success:function(result){
            console.log(result.data)
...
function createSeriPajak() {
    ...
    $.ajax({
        type:"POST",
        url:base_url+"Rekapoutlet/get_seripajak",
        dataType:"json",
        data:so_item,
        success:function(result){
            console.log(result.data)
...
function printSOss() {
    window.open(base_url+'Rekapoutlet/print_so/'+docno, '_blank');
}
...
function printSO() {
    ...
    $.messager.confirm({
        ...
        fn: function(r){
            if (r){
                let urlss = `${base_url}Rekapoutlet/print_ws?id=${so_item.id}&tipe=${print_selected.label}&pkp=${so_item.pkp}`;
...
function initGrid() {
    ...
    $("#dg").edatagrid({
        fitColumns: false,
        width: "100%",
        url: base_url + "Rekapoutlet/load_grid_detail/"+so_item.id,
        saveUrl: base_url + "Rekapoutlet/save_data_detail/"+so_item.id,
        updateUrl: base_url + "Rekapoutlet/edit_data_detail",
        destroyUrl: base_url + "Rekapoutlet/delete_data_detail",
        idField: 'id',
        method: "POST",
        pagePosition: "top",
        resizeHandle: "right",
        ...
        onBeginEdit: function (index, row) {
            if (row.isNewRecord ){
                if(so_item.status!=="OPEN") {
                    $.messager.show({
                        title: 'Warning',
                        msg: "Rekapoutlet sudah di posting"
                    });
                    setTimeout(function () {
    ...
    if(aksi==="add"){
        ...
        if(so_item!==undefined){
            if(so_item.status==="CLOSED" && status==="OPEN"){
                ...
                myConfirm("Confirm", "Anda yakin ingin mengubah status Rekapoutlet ini?", "Yes", "No", function (r) {
                    console.log(r)
                    if (r === "Yes") {
                        submit_reason("")
                    }
                ...
                if (res.status === 0) {
                    ...
                    if(stt==="POSTING") {
                        myConfirm("Success", "Posting berhasil, apakah anda ingin mencetak doc.?", "Cetak", "Tidak", function (r) {
                            if (r === "Cetak") {
                                printSO()
                                window.location.href = base_url + "Rekapoutlet/form/edit?id=" + res.id
                            }else window.location.href = base_url + "Rekapoutlet/form/edit?id=" + res.id
...
function populateJenisSO() {
    ...
    $('#jenis_faktur').combobox('setValue','CONSIGNMENT')
}
