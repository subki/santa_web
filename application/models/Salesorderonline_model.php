...
class Salesorderonline_model extends CI_Model {

    private $table;
    private $query;
	public function __construct(){
        parent::__construct();
        $this->table = "so_online_header";
...
//        $this->db->query($sql);
    ...
    function delete_data_detail($docno, $id){
        $this->db->where('id',$id);
        $this->db->delete("so_online_detail");
        $this->updateheaderdata($docno);
    }
    ...
    function updateheaderdata($docno){
        $sql = "UPDATE so_online_header AS dest
                , (SELECT (  SELECT COUNT(nobar) FROM
                    ( SELECT DISTINCT nobar FROM so_online_detail WHERE docno = '$docno'
                        ...
                        , SUM(CEILING(so_online_detail.net_total_price*so_online_detail.qty_order)) after_tax
