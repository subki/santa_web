...
class Salesorder_model extends CI_Model {
	...
	public function __construct(){
        ...
        $this->query = "select a.docno
                  ...
                  , a.store_code, b.store_name, a.location_code, g.description location_name, a.kode_kirim, a.provinsi_id, c.name as provinsi
                  ...
                  , a.tipe_komisi, a.komisi_persen, a.disc1_persen, a.disc2_persen, a.disc3_persen
    ...
    function generate_auto_number($pkp, $store_code){
        $prefix = "";
        if($pkp=="YES"){
...
//                    'net_total_price' => $input['net_total_price'],
	    if($nomor==1){
            $this->db->query("update sales_order_detail set disc1_persen=$disc where docno='$docno'");
        }else if($nomor==2){
            $this->db->query("update sales_order_detail set disc2_persen=$disc where docno='$docno'");
        }else if($nomor==3){
            $this->db->query("update sales_order_detail set disc3_persen=$disc where docno='$docno'");
        }
...
//	              where docno='$docno'";
//        $this->db->query($sql);
        $sql = "update sales_order_detail set disc1_amount=unit_price * (disc1_persen/100) where docno='$docno'";
        if($this->db->query($sql)) {
            $sql = "update sales_order_detail set disc2_amount=(unit_price-disc1_amount) * (disc2_persen/100) where docno='$docno'";
            if($this->db->query($sql)){
                $sql = "update sales_order_detail set disc3_amount=(unit_price-disc1_amount-disc2_amount) * (disc3_persen/100) where docno='$docno'";
                if($this->db->query($sql)){
                    $sql = "update sales_order_detail set disc_total=disc1_amount+disc2_amount+disc3_amount where docno='$docno'";
                    if($this->db->query($sql)){
                        $sql = "update sales_order_detail set bruto_before_tax=unit_price-disc_total where docno='$docno'";
                        if($this->db->query($sql)){
                            $sql = "update sales_order_detail set total_tax=(case when '$pkp'='YES' then (bruto_before_tax*qty_order/1.1)*10/100 else 0 end) where docno='$docno'";
                            if($this->db->query($sql)){
                                $sql = "update sales_order_detail set net_unit_price=unit_price-disc_total where docno='$docno'";
                                if($this->db->query($sql)){
                                    $sql = "update sales_order_detail set net_total_price=(unit_price-disc_total)*qty_order where docno='$docno'";
                                    if($this->db->query($sql)){
                                        $this->updateheaderdata($docno);
                                    }
                                }
...
//        return $this->db->get('product');
    ...
    function copy_detail($from, $to, $user, $tgl, $pkp){
        $sql = "insert into sales_order_detail (docno, seqno, nobar, tipe, crtby, crtdt
                , unit_price, disc1_persen, disc1_amount, disc2_persen, disc2_amount, disc3_persen, disc3_amount
                , disc_total, bruto_before_tax, total_tax, net_unit_price, net_total_price)
                select '$to', a.seqno, a.nobar, a.tipe, '$user', '$tgl'
                 , a.unit_price, b.disc1_persen, 0, b.disc2_persen, 0, b.disc3_persen, 0
                 , 0, 0, 0, 0, 0 FROM sales_order_detail a
                 LEFT JOIN sales_order_header b ON b.docno='$to' 
                 WHERE a.docno='$from'";
        if($this->db->query($sql)){
            $sql = "update sales_order_detail set disc1_amount=unit_price * (disc1_persen/100) where docno='$docno'";
            if($this->db->query($sql)) {
