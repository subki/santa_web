...
class Salesorderoutlet_model extends CI_Model {

    private $table;
    private $query;
	public function __construct(){
        parent::__construct();
        $this->table = "so_outlet_header";
    ...
    function read_totaldetail($code){
        $q = "SELECT count(*) total from so_outlet_detail where docno='$code'";
        return $this->db->query($q);
    }
...
//         if($prefix=="") return "";
        $sql = "SELECT IFNULL(CONCAT(DATE_FORMAT(NOW(),'%y%m%d'),LPAD(MAX(RIGHT(docno,6))+1,6,'0')),
                ...
                FROM so_outlet_header WHERE LEFT(docno,6)= CONCAT(DATE_FORMAT(NOW(),'%y%m%d')) ORDER BY docno DESC";
    ...
    function cek_detail($docno, $nobar, $tipe){
        ...
        return $this->db->get('so_outlet_detail');
    }
    ...
    function update_data_detail($docno,$code, $data){
        ...
        $this->db->update("so_outlet_detail",$data);
...
//                    'net_total_price' => $input['net_total_price'],
	    if($nomor==1){
            $this->db->query("update so_outlet_detail set disc1_persen=$disc where docno='$docno'");
        }else if($nomor==2){
            $this->db->query("update so_outlet_detail set disc2_persen=$disc where docno='$docno'");
        } 
//        $sql = "update so_outlet_detail set
...
//	              where docno='$docno'";
//        $this->db->query($sql);
        $sql = "update so_outlet_detail set disc1_amount=unit_price * (disc1_persen/100) where docno='$docno'";
        if($this->db->query($sql)) {
            $sql = "update so_outlet_detail set disc2_amount=(unit_price-disc1_amount) * (disc2_persen/100) where docno='$docno'";
             if($this->db->query($sql)){
                        $sql = "update so_outlet_detail set bruto_before_tax=unit_price-disc_total where docno='$docno'";
                        if($this->db->query($sql)){
                            $sql = "update so_outlet_detail set total_tax=(case when '$pkp'='YES' then (bruto_before_tax*qty_order/1.1)*10/100 else 0 end) where docno='$docno'";
                            if($this->db->query($sql)){
                                $sql = "update so_outlet_detail set net_unit_price=unit_price-disc_total where docno='$docno'";
                                if($this->db->query($sql)){
                                    $sql = "update so_outlet_detail set net_total_price=(unit_price-disc_total)*qty_order where docno='$docno'";
                                    if($this->db->query($sql)){
                                        $this->updateheaderdata($docno);
                                    }
                                }
    ...
    function insert_data_detail($docno,$data){ 
        $this->db->insert("so_outlet_detail", $data);
        $this->updateheaderdata($docno);
    }
    function delete_data_detail($docno, $id){
        $this->db->where('id',$id);
        $this->db->delete("so_outlet_detail");
        $this->updateheaderdata($docno);
    }
...
//        return $this->db->get('product');
    ...
    function updateheaderdata($docno){
        $sql = "UPDATE so_outlet_header AS dest
                , (SELECT (  SELECT COUNT(nobar) FROM
                    ( SELECT DISTINCT nobar FROM so_outlet_detail WHERE docno = '$docno'
                    ) AS item) item, SUM(qty_order) qty
                        , SUM(CEILING(so_outlet_detail.unit_price*so_outlet_detail.qty_order)) bruto
                        , SUM(CEILING(so_outlet_detail.disc_total*so_outlet_detail.qty_order)) disc
                        , SUM(CEILING(so_outlet_detail.bruto_before_tax)) before_tax
                        , SUM(CEILING(so_outlet_detail.net_total_price*so_outlet_detail.qty_order)) after_tax
                        , SUM(CEILING(so_outlet_detail.total_tax)) ppn
                  FROM so_outlet_detail WHERE docno='$docno') AS src
    ...
    function copy_detail($from, $to, $user, $tgl, $pkp){
        $sql = "insert into so_outlet_detail (docno, seqno, nobar, tipe, crtby, crtdt
                ...
                select '$to', a.seqno, a.nobar, a.tipe, '$user', '$tgl'
                 ...
                 , 0, 0, 0, 0, 0 FROM so_outlet_detail a
                 LEFT JOIN so_outlet_header b ON b.docno='$to' 
                 WHERE a.docno='$from'";
        if($this->db->query($sql)){
            $sql = "update so_outlet_detail set disc1_amount=unit_price * (disc1_persen/100) where docno='$docno'";
            if($this->db->query($sql)) {
