...
class Productsize extends IO_Controller {

    function __construct(){

        parent::__construct();
        $this->load->model('Productsize_model','model');
    ...
    function index(){
        $data['title']      = 'Size';
        $data['content']    = $this->load->view('vProductsize',$data,TRUE);
    ...
    function load_grid(){
        ...
        $sort = ($this->input->post('sort')) ? $this->input->post('sort'):'size_code';
    ...
    function save_data(){
        try {
            ...
            $read = $this->model->read_data($input['size_code']);
            if ($read->num_rows() > 0) {
                $result = 1;
                ...
                if($this->checkChar($input['size_code'])){
    ...
    function edit_data(){
        try {
            ...
            if ($read->num_rows() > 0) {
                ...
                $this->model->update_data($input['size_code'], $data);
                $result = 0;
                $msg="OK";
            } else {
                $result = 1;
