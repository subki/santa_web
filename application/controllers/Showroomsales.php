...
class Showroomsales extends IO_Controller {

    function __construct(){

        parent::__construct();
        $this->load->model('Showroomsales_model','model');
        $this->load->library('form_validation');
        $this->load->helper('file');
    }

    function index($aksi=""){
        $data['title'] = 'Sales Order Showroomsales';
        $data['content'] = $this->load->view('vShowroomsales', $data, TRUE);
    ...
    function form($aksi=""){
        $data['aksi']=$aksi;
        if($aksi=="add"){
            $data['title'] = 'Add Sales Order Showroomsales';
            ...
            $data['content'] = $this->load->view('vShowroomsales_form', $data, TRUE);
        }else{
            $data['title'] = 'Edit Sales Order Showroomsales';
...
//                    'posting_date' => $this->formatDate("Y-m-d",$input['posting_date']),
                ...
                $this->insert_log("so_Showroomsales_header", $docno, "Add Header Data");
                $result = 0;
                $msg = "OK";
                $docno = $docno;
            }
...
//                    'qty_deliver' => $input['qty_deliver'],
    ...
    function print_so($docno){
        ...
        $this->pdf->load_view('print/Showroomsales', $data);
        $this->pdf->render();
    ...
    function save_data_detail($docno){
        try {
            ...
            $seqno = $this->model->generate_seqno($docno);
                ...
                // var_dump($data);
                // die();
                $this->insert_log("sales_order_Showroomsales_detail", $docno, "Add detail Data ".$input['nobar']);
