...
class Packinglist extends IO_Controller {
	...
	function approval(){
		$data['title'] = 'Packing List Approval Limit Maximum Sales';
		$data['content'] = $this->load->view('vPackinglistapp', $data, TRUE);
		$this->load->view('main',$data);
	}
	...
	function load_grid(){
		$total1 = $this->getParamGrid_BuilderComplete(array(
			...
			"select"=>"a.docno
                  ...
                  , ifnull(st.id,0) as stid, ifnull(st.no_faktur,'') as stno 
                  , (select sum(sd.net_total_price/sd.qty_order*pl.qty_pl) from sales_order_detail sd
                  	inner join packing_detail pl on pl.so_number=sd.docno and pl.seqno=sd.seqno
                   	where sd.docno=a.so_number) as sales_total_pl",
			"join"=>[
				...
				"sales_trans_header st"=>"st.base_so=a.docno",
			...
			if ($read->num_rows() > 0) {
				...
				if($input['status']=="POSTING"){
					...
					if($so->status=="ON ORDER") {
						$pl_cek = $this->db->select("pl.*, sd.net_total_price")
							->from("packing_detail pl")
							->join("sales_order_detail sd","sd.docno=pl.so_number and sd.seqno=pl.seqno")
							->where("pl.docno",$input['docno'])->get()->result();
						if(count($pl_cek)){
							$total_sales_packing = 0;
							$lanjut = true;
							foreach ($pl_cek as $r){
								if($r->qty_pl==0){
									$lanjut = false; break;
								}
								$total_sales_packing += $r->net_total_price/$r->qty_order*$r->qty_pl;
							}
							if($lanjut) {
								if ($total_sales_packing < $this->session->userdata('maksimal transaksi')) {
									$data['posting_date'] = date('Y-m-d');
								} else {
									$err = 1;
									$result = 1;
									$msg = "Posting Packing List membutuhkan Approval Limit Maksimum Sales : ".number_format($total_sales_packing);
								}
							}else{
								$err = 1;
								$result = 1;
								$msg = "Qty Packing list harus di isi semua!";
							}
						}else{
							$err = 1;
							$result = 1;
							$msg="Detail Packing list tidak ditemukan!";
						}
					}else{
						$err = 1;
						$result = 1;
		...
		try {
			...
			if ($read->num_rows() > 0) {
				...
				if($err==0) {
					...
					$input['approvedby'] = $this->session->userdata(sess_user_id);
					$input['approveddt'] = date('Y-m-d H:i:s');
					$this->model->update_data($input['docno'], $input);
					$msg = "Packing list limit maksimum sales berhasil di approve";
	...
	function load_grid_detail($docno){
		$total1 = $this->getParamGrid_BuilderComplete(array(
			...
			"select"=>"a.id, a.docno, a.seqno, a.nobar, a.qty_order, a.qty_pl
                    ...
                    , (sd.net_total_price / sd.qty_order * a.qty_pl) as net_price_pl",
			"join"=>[
